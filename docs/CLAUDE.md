...
- Tailwind CSS v4 (CSS-first konfiguracija, `@tailwindcss/vite` plugin, nema `tailwind.config.js`)
...
- `tailwind.config.js` za konfiguraciju boja/fontova — koristiti `@theme` u `index.css`
- PostCSS za Tailwind — koristiti `@tailwindcss/vite` plugin u `vite.config.ts`
- Pravi pacijentski podaci
