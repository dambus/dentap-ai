...
export function Sidebar() {
  ...
  return (
    <TooltipProvider>
      <aside className="flex flex-col w-60 shrink-0 h-screen bg-white border-r border-slate-200">
          ...
          {NAV_ITEMS.map(({ href, label, icon: Icon }) => (
            ...
            >
              {({ isActive }) => (
                <>
                  <Icon
                    className={cn('w-4 h-4 shrink-0', isActive ? 'text-teal-600' : 'text-slate-400')}
                  />
                  {label}
                </>
              )}
        ...
        <div className="px-2 pb-3 border-t border-slate-200 pt-3">
          <div className="flex items-center gap-2.5 px-2 py-1.5 rounded-md">
            ...
            <Tooltip content="Odjavi se" side="right">
              <button
                onClick={signOut}
                className="p-1.5 rounded text-slate-400 hover:text-slate-700 hover:bg-slate-100 transition-colors focus:outline-none focus:ring-2 focus:ring-teal-600"
              >
